...
HelpLibraryManagerLauncher.exe /product "VS" /version "100" /locale en-us /uninstall /silent /vendor "Vendor Name" /productName "EwE tools for MSP - API documentation reference" /mediaBookList "EwE tools for MSP - API documentation reference" > NUL
...
REM build time.
IF EXIST "EwE tools for MSP API documentation reference.msha" COPY /Y "EwE tools for MSP API documentation reference.msha" HelpContentSetup.msha
...
REM Uninstall first in case it is already there.  If not, it won't install below.  We'll ignore any error output
REM by redirecting it to NUL.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation uninstall /vendor "Vendor Name" /productName "EwE tools for MSP - API documentation reference" /bookList "EwE tools for MSP - API documentation reference" > NUL

REM Install the new content.
HelpLibraryManagerLauncher.exe /viewerVersion %1  /locale en-us /wait 0 /operation install /sourceUri "%CD%\EwE tools for MSP API documentation reference.msha"
