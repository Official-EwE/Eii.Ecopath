...
Public Class cDynamicsBasicParam
...
#Region "Helper methods"
    ...
    Private Shared Sub SetUpGridRowPropertyDefault(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager)
        DataGrid.RowCount = EcotrophManager.DynamicsInputBiomass.GetUpperBound(0)
    ...
    Private Shared Sub SetUpGridRow(ByVal PanelToolStrip As Panel, ByVal DataGrid As DataGridView, _
      ByVal EcotrophManager As cEcotrophManager)
        ...
        TLOut = 2
        For Row As Integer = 1 To EcotrophManager.DynamicsInputBiomass.GetUpperBound(0)
            ...
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputBiomass(Row)) Then
                RowContent(2) = ""
            Else
                RowContent(2) = EcotrophManager.DynamicsInputBiomass(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputAccessBiomass(Row)) Then
                RowContent(3) = ""
            Else
                RowContent(3) = EcotrophManager.DynamicsInputAccessBiomass(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputFlow(Row)) Then
                RowContent(4) = ""
            Else
                RowContent(4) = EcotrophManager.DynamicsInputFlow(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputKinetic(Row)) Then
                RowContent(5) = ""
            Else
                RowContent(5) = EcotrophManager.DynamicsInputKinetic(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputCatches(Row)) Then
                RowContent(6) = ""
            Else
                RowContent(6) = EcotrophManager.DynamicsInputCatches(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputFishLossRate(Row)) Then
                RowContent(7) = ""
            Else
                RowContent(7) = EcotrophManager.DynamicsInputFishLossRate(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputAccessFishLossRate(Row)) Then
                RowContent(8) = ""
            Else
                RowContent(8) = EcotrophManager.DynamicsInputAccessFishLossRate(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputFishMortality(Row)) Then
                RowContent(10) = ""
            Else
                RowContent(10) = EcotrophManager.DynamicsInputFishMortality(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputAccessFishMortality(Row)) Then
                RowContent(11) = ""
            Else
                RowContent(11) = EcotrophManager.DynamicsInputAccessFishMortality(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.DynamicsInputSelectivity(Row)) Then
                RowContent(12) = ""
            Else
                RowContent(12) = EcotrophManager.DynamicsInputSelectivity(Row).ToString("F4")
            End If
            If Row < EcotrophManager.DynamicsInputBiomass.GetUpperBound(0) Then
                If Single.IsNaN(EcotrophManager.DynamicsInputNaturalLossRate(Row)) Then
                    RowContent(9) = ""
                Else
                    RowContent(9) = EcotrophManager.DynamicsInputNaturalLossRate(Row).ToString("F4")
                End If
                If Single.IsNaN(EcotrophManager.DynamicsInputTime(Row)) Then
                    RowContent(13) = ""
                Else
                    RowContent(13) = EcotrophManager.DynamicsInputTime(Row).ToString("F4")
                End If
            Else
                RowContent(9) = ""
                RowContent(13) = ""
