'==============================================================================
'
' $Log: cAccessBiomass.vb,v $
' Revision 1.1  2008/09/26 07:30:41  sherman
' --== DELETED HISTORY ==--
'
' Revision 1.15  2008/06/05 19:43:45  joeh
' no message
'
'==============================================================================
Option Explicit On
...
#Region "Helper methods"
    ...
    Private Shared Sub SetUpToolStrip(ByVal PanelToolStrip As Panel, ByVal EcotrophManager As cEcotrophManager, _
      ByVal Algor As String)
        ...
        Select Case Algor
            Case My.Resources.TREE_NODE_AUTO_SMOOTH ' My.Resources.TREE_NODE_AUTO_EMPIR_FUNCT
            ...
            Case My.Resources.TREE_NODE_USER_DEF_SIGMA, My.Resources.TREE_NODE_OMNI_IDX
                '
        End Select
        ToolStp.Refresh()
    ...
    Private Shared Sub SetUpGridColumnPropertyDefault(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager)
        ...
        DataGrid.ColumnCount = EcotrophManager.AccessBiomass.GetUpperBound(1) + 2
      ...
      ByVal Algor As String)
        Select Case Algor
            Case My.Resources.TREE_NODE_AUTO_SMOOTH
                DataGrid.RowCount = EcotrophManager.AccessBiomass.GetUpperBound(0) + 1
            Case My.Resources.TREE_NODE_OMNI_IDX, My.Resources.TREE_NODE_USER_DEF_SIGMA
                DataGrid.RowCount = EcotrophManager.AccessBiomass.GetUpperBound(0) + 2
        End Select
      ...
      ByVal Algor As String)
        ...
        Select Case Algor
            Case My.Resources.TREE_NODE_AUTO_SMOOTH
                'Set up Access row
                RowContent(0) = ""
                RowContent(1) = My.Resources.CELL_ACCESS_TRP_LVL
                For Col As Integer = 1 To EcotrophManager.AccessBiomass.GetUpperBound(1)
                    RowContent(Col + 1) = EcotrophManager.InputData.Access(Col).ToString("0.#")
                Next
                DataGrid.Rows(0).SetValues(RowContent)
                DataGrid.Rows(0).Height = cUtility.ACCESS_TRP_LVLOUT_ROW_HEIGHT
            ...
            Case My.Resources.TREE_NODE_OMNI_IDX
                'Set up Sigma row
                RowContent(0) = ""
                RowContent(1) = My.Resources.CELL_SIGMA
                For Col As Integer = 1 To EcotrophManager.AccessBiomass.GetUpperBound(1)
                ...
                DataGrid.Rows(1).Height = cUtility.ACCESS_TRP_LVLOUT_ROW_HEIGHT
    ...
    Private Shared Sub SetUpAryRow(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager, _
      ByVal Algor As String)
        ...
        TLOut = 2
        For Row As Integer = 1 To EcotrophManager.AccessBiomass.GetUpperBound(0)
            ...
            End If
            For Col As Integer = 1 To EcotrophManager.AccessBiomass.GetUpperBound(1)
                If Single.IsNaN(EcotrophManager.AccessBiomass(Row, Col)) Then
                    RowContent(Col + 1) = ""
                Else
                    RowContent(Col + 1) = (EcotrophManager.AccessBiomass(Row, Col)).ToString("F4")
                End If
            Next
            Select Case Algor
                Case My.Resources.TREE_NODE_AUTO_SMOOTH
