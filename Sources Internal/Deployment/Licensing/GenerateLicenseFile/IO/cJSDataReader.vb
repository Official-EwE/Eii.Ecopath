...
#Region " Loading "
    ...
    ''' <param name="strFilter">An optional filter, such as Excel worksheet name, Access database table, etc. This second value depends on the format of the file that is loaded.</param>
    ''' <returns>True if successful.</returns>
    Public Shared Function Read(Of T)(strFile As String, strFilter As String, data As BindingList(Of T),
                                      Optional ColumnTypes As Dictionary(Of String, Type) = Nothing,
                                      Optional bAppend As Boolean = False) As Boolean
        Return cDataTableConverter.ToBindingList(Read(strFile, strFilter, ColumnTypes), data, bAppend)
    End Function
...
#Region " Text "
    ...
    Private Shared Function LoadText(sr As TextReader,
        ...
        Try
            ...
            Dim strArray() As String = strLine.Split(TextFieldSeparator)
            Dim dt As New DataTable()
            ...
            While Not String.IsNullOrEmpty(strLine)
                row = dt.NewRow()
                ' Parse
                strArray = strLine.Split(TextFieldSeparator)
