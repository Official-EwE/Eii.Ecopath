'==============================================================================
'
' $Log: cProportionSTD.vb,v $
' Revision 1.1  2008/09/26 07:30:41  sherman
' --== DELETED HISTORY ==--
'
' Revision 1.14  2008/06/05 19:43:45  joeh
' no message
'
'==============================================================================
Option Explicit On
...
#Region "Helper methods"
    ...
    ByVal ToolStp As ToolStrip, ByVal Algor As String)
        Select Case Algor
            Case My.Resources.TREE_NODE_AUTO_SMOOTH, My.Resources.TREE_NODE_USER_DEF_SIGMA
    ...
    Private Shared Sub SetUpGridColumnPropertyDefault(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager)
        ...
        DataGrid.ColumnCount = EcotrophManager.ProportionSTD.GetUpperBound(1) + 2
    ...
    Private Shared Sub SetUpGridRowPropertyDefault(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager, _
      ByVal Algor As String)
        Select Case Algor
            Case My.Resources.TREE_NODE_AUTO_SMOOTH
                DataGrid.RowCount = EcotrophManager.ProportionSTD.GetUpperBound(0)
            Case My.Resources.TREE_NODE_OMNI_IDX, My.Resources.TREE_NODE_USER_DEF_SIGMA
                DataGrid.RowCount = EcotrophManager.ProportionSTD.GetUpperBound(0) + 1
        End Select
    ...
    Private Shared Sub SetUpGridCellPropertyDefault(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager)
        ...
        For Col As Integer = 1 To EcotrophManager.ProportionSTD.GetUpperBound(1)
    ...
    Private Shared Sub SetUpGridColumn(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager, _
      ByVal Algor As String)
        ...
        Select Case Algor
            Case My.Resources.TREE_NODE_AUTO_SMOOTH
                DataGrid.Columns(1).HeaderText = My.Resources.COL_HDR_TRP_LVLINSIGMA_TRP_LVLOUT
                DataGrid.Columns(2).HeaderText = "1"
                Col = 1
                For TLIn As Single = 2 To 5 Step 0.1
                    DataGrid.Columns(Col + 2).HeaderText = TLIn.ToString("0.#") & Chr(10) & " (" _
                      & EcotrophManager.SigmaLN(Col).ToString("F4") & ")"
                    Col = Col + 1
                Next
            Case My.Resources.TREE_NODE_OMNI_IDX, My.Resources.TREE_NODE_USER_DEF_SIGMA
                DataGrid.Columns(1).HeaderText = My.Resources.COL_HDR_GRP_NAMETL
                For Col = 1 To EcotrophManager.EcopathData.NumLiving
    ...
    Private Shared Sub SetUpAryRow(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager, _
      ByVal Algor As String)
        ...
        TLOut = 2
        For Row As Integer = 1 To EcotrophManager.ProportionSTD.GetUpperBound(0)
            ...
            End If
            For Col As Integer = 1 To EcotrophManager.ProportionSTD.GetUpperBound(1)
                If Single.IsNaN(EcotrophManager.ProportionSTD(Row, Col)) Then
                    RowContent(Col + 1) = ""
                Else
                    RowContent(Col + 1) = (EcotrophManager.ProportionSTD(Row, Col)).ToString("F4")
                End If
            Next
            Select Case Algor
                Case My.Resources.TREE_NODE_AUTO_SMOOTH
...
#End Region 'Helper methods
    ...
    '                    Col = Col + 1
    '                Next
    '            Case My.Resources.TREE_NODE_OMIN_IDX
    '                '
    '        End Select
    '    End Sub
