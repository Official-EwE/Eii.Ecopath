...
#Region " Vars "
    ...
    ' ToDo: reroute this to the central value chain engine running here.

    Private m_engine As cValueChain = Nothing
...
#Region " Constructor "
    ...
    Public Sub New(uic As cUIContext, engine As cValueChain)
    ...
    Public Sub ShowForm(page As eValueChainPageTypes)
        ...
        Select Case Me.m_pageCurrent
            Case eValueChainPageTypes.Parameters
                Me.ShowForm(New ucParameters(Me.m_engine.Data, Me.m_uic), My.Resources.NAVTREE_INPUT_PARAMETERS)
            Case eValueChainPageTypes.TableProducers
                Dim grid As New ucUnitGrid(Me.m_uic, Me.m_engine.Data, cUnitFactory.eUnitType.Producer)
            ...
            Case eValueChainPageTypes.TableProcessors
                Dim grid As New ucUnitGrid(Me.m_uic, Me.m_engine.Data, cUnitFactory.eUnitType.Processing)
                Dim view As New ucGridView(grid)
                Me.ShowForm(view, My.Resources.NAVTREE_INPUT_TABLE_PROCESSORS)
            Case eValueChainPageTypes.TableDistributors
                Dim grid As New ucUnitGrid(Me.m_uic, Me.m_engine.Data, cUnitFactory.eUnitType.Distribution)
                Dim view As New ucGridView(grid)
                Me.ShowForm(view, My.Resources.NAVTREE_INPUT_TABLE_DISTRIBUTORS)
            Case eValueChainPageTypes.TableWholesellers
                Dim grid As New ucUnitGrid(Me.m_uic, Me.m_engine.Data, cUnitFactory.eUnitType.Wholesaler)
                Dim view As New ucGridView(grid)
                Me.ShowForm(view, My.Resources.NAVTREE_INPUT_TABLE_WHOLESALERS)
            Case eValueChainPageTypes.TableRetailer
                Dim grid As New ucUnitGrid(Me.m_uic, Me.m_engine.Data, cUnitFactory.eUnitType.Retailer)
                Dim view As New ucGridView(grid)
                Me.ShowForm(view, My.Resources.NAVTREE_INPUT_TABLE_RETAILERS)
            Case eValueChainPageTypes.TableConsumers
                Dim grid As New ucUnitGrid(Me.m_uic, Me.m_engine.Data, cUnitFactory.eUnitType.Consumer)
                Dim view As New ucGridView(grid)
                Me.ShowForm(view, My.Resources.NAVTREE_INPUT_TABLE_CONSUMERS)
            Case eValueChainPageTypes.Flow
                Me.ShowForm(New ucEditFlow(Me.m_uic, Me.m_engine.Data, Me.m_engine.Data.FlowDiagram(0)), My.Resources.NAVTREE_INPUT_FLOW)
            Case eValueChainPageTypes.Defaults
                Me.ShowForm(New ucDefaults(Me.m_uic, Me.m_engine.Data), My.Resources.NAVTREE_INPUT_DEFAULTS)
            Case eValueChainPageTypes.TableLinks
                Dim grid As New ucLinkGrid(Me.m_uic, Me.m_engine.Data, GetType(cLink))
                Dim view As New ucGridView(grid)
                Me.ShowForm(view, My.Resources.NAVTREE_INPUT_TABLE_LINKS)
            Case eValueChainPageTypes.TableLandingsLinks
                Dim grid As New ucLinkGrid(Me.m_uic, Me.m_engine.Data, GetType(cLinkLandings))
                Dim view As New ucGridView(grid)
                Me.ShowForm(view, My.Resources.NAVTREE_INPUT_TABLE_LANDINGLINKS)
            Case eValueChainPageTypes.Run
                Me.ShowForm(New ucResults(Me.m_uic, Me.m_engine.Data, Me.m_engine.Model, Me.m_engine.Results), My.Resources.NAVTREE_OUTPUT_RUN)
            Case eValueChainPageTypes.FlowDiagram
                Me.ShowForm(New ucFlowDiagram(Me.m_uic, Me.m_engine.Data, Me.m_engine.Model, Me.m_engine.Results), My.Resources.NAVTREE_OUTPUT_FLOWDIAGRAM)
            Case Else
                Debug.Assert(False)
        End Select
    ...
    Private Sub ShowForm(f As Control, strTitle As String)
        ...
        Try
            If TypeOf f Is IUIElement Then
                DirectCast(f, IUIElement).UIContext = Me.m_uic
            End If
