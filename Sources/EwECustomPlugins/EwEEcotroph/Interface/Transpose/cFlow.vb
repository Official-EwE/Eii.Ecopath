...
#Region "Helper methods"
    Private Shared Sub SetUpGridColumn(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager)
        DataGrid.ColumnCount = EcotrophManager.Flow.GetUpperBound(1) + 2
        ...
        DataGrid.Columns(1).HeaderText = My.Resources.COL_HDR_GRP_NAMEPB_TRP_LVLOUT
        For Col As Integer = 1 To EcotrophManager.EcopathData.NumLiving
            DataGrid.Columns(Col + 1).HeaderText = EcotrophManager.EcopathData.GroupName(Col) & _
              Chr(10) & " (" & EcotrophManager.ProductionBiomass(Col) & ")"
        Next
    End Sub
    ...
    Private Shared Sub SetUpGridRow(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager)
        ...
        DataGrid.RowCount = EcotrophManager.Flow.GetUpperBound(0)

        ReDim RowContent(DataGrid.Columns.Count)
        TLOut = 2
        For Row As Integer = 1 To EcotrophManager.Flow.GetUpperBound(0)
            ...
            End If
            For Col As Integer = 1 To EcotrophManager.Flow.GetUpperBound(1)
                RowContent(Col + 1) = (EcotrophManager.Flow(Row, Col)).ToString("F4")
            Next
