...
#Region " Flow management "
    ...
    Public Sub RebuildFlow()
        ...
        ElseIf (Me.ItemFilter IsNot Nothing) Then
            ' #No: Has fleet filter?
            ' ToDo: need to rewrite this logic, where item filter is resolved in UI (not in data)
            lUnits.AddRange(Me.GetUnits(m_data, Me.ItemFilter))
        Else
            ' #No: grab all units
    ...
    Public Sub CreateProducersForFleets()
        ...
        For iFleet As Integer = 1 To core.nFleets
            ...
            If Not bProducerExists Then
                ...
                ' ToDo: determine fleet malpping here
                pu.Fleet = fleet
                pu.AllowEvents = True
            End If
        Next iFleet
...
#Region " Internals "
    ...
    ''' <returns></returns>
    ''' -----------------------------------------------------------------------
    Public Function GetUnits(data As cData, item As cCoreInputOutputBase) As cUnit()

        Dim lUnits As New List(Of cUnit)
        Dim pu As cProducerUnit = Nothing
        Dim bUseUnit As Boolean = False

        If (item Is Nothing) Then Return data.GetUnits(cUnitFactory.eUnitType.All)
        ...
        For Each unit As cUnit In data.GetUnits(cUnitFactory.eUnitType.Producer)
            ...
            Else
                ...
                ' ToDo: centrally resolve fleet and group between EwE and Value Chain
            ...
            If bUseUnit Then
                ...
                Me.GetTargetUnits(data, pu, lUnits)
            End If
    ...
    ''' -----------------------------------------------------------------------
    Private Sub GetSourceUnits(data As cData, unit As cUnit, lUnits As List(Of cUnit))
        ...
        For iLink As Integer = 0 To unit.LinkInCount - 1
            ...
            If lUnits.IndexOf(unitSource) = -1 Then
                ...
                Me.GetSourceUnits(data, unitSource, lUnits)
            End If
        Next
    End Sub
    ...
    ''' <param name="lUnits">The list that will receive the linked units.</param>
    ''' -----------------------------------------------------------------------
    Private Sub GetTargetUnits(data As cData, unit As cUnit, lUnits As List(Of cUnit))
        ...
        For iLink As Integer = 0 To unit.LinkOutCount - 1
            ...
            If lUnits.IndexOf(unitTarget) = -1 Then
                ...
                Me.GetTargetUnits(data, unitTarget, lUnits)
            End If
        Next
    End Sub
