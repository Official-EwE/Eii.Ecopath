    ...
    Private Function GetLinks() As cLink()
        ...
        For Each l As cLink In Me.m_data.GetLinks(Me.m_typeData)
            ...
            If (TypeOf l Is cLinkLandings) Then
                ...
                ' ToDo: redesign EwE - Value Chain group and fleet mapping, also in the light of SURIMI
                If (Me.m_group IsNot Nothing) Then
                    bUse = bUse And (String.Compare(ll.Species, CStr(Me.m_group.DBID), True) = 0)
                End If
                If (Me.m_fleet IsNot Nothing) Then
                    bUse = bUse And (String.Compare(DirectCast(ll.Source, cProducerUnit).Fleet, CStr(Me.m_fleet.DBID)) = 0)
                End If
            End If
