...
'    ''' <summary>Value chain results computed for an ecossytem snapshot.</summary>
'    Snapshot
'    ''' <summary>Value chain results computed over time.</summary>
'    Temporal
...
#Region " Private vars "
    ...
    Private m_nTimeSteps As Integer = 1
...
#Region " Public access "
    ...
    ''' -----------------------------------------------------------------------
    Public Sub Reset(nFleets As Integer, nGroups As Integer, nTimesteps As Integer)

        Dim nNumUnits As Integer = Me.m_data.GetUnits(cUnitFactory.eUnitType.All).Length
        Dim nItems As Integer = Math.Max(nFleets, nGroups)

        Me.m_dtResultTimeStep.Clear()
        Me.m_dtSnapshots.Clear()
        Me.m_iMaxTimeStep = 0
        Me.m_iMaxItem = 0
        Me.m_nTimeSteps = nTimesteps

        ReDim Me.m_ValueContribution(nItems, nNumUnits, Math.Max(1, nTimesteps))
        ReDim Me.m_BiomassContribution(nItems, nNumUnits, Math.Max(1, nTimesteps))
...
#Region " Contribution by item "
    ...
    ''' </remarks>
    ''' -----------------------------------------------------------------------
    Public Sub StoreContribution(iItem As Integer, unit As cUnit, iTimeStep As Integer, sValueContribution As Single, sBiomassContribution As Single)
