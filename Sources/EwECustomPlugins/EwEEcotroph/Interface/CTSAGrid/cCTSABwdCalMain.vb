'==============================================================================
'
' $Log: cCTSABwdCalMain.vb,v $
' Revision 1.1  2008/09/26 07:30:38  sherman
' --== DELETED HISTORY ==--
'
' Revision 1.7  2008/06/05 19:43:46  joeh
' no message
'
'==============================================================================
Option Explicit On
...
#Region "Helper methods"
    ...
    Private Shared Sub SetUpGridRow(ByVal DataGrid As DataGridView, ByVal EcotrophManager As cEcotrophManager)
        ...
        TLOut = 2
        For Row As Integer = 1 To EcotrophManager.BwdCalBiomass.GetUpperBound(0)
            ...
            If Single.IsNaN(EcotrophManager.BwdCalBiomass(Row)) Then
                RowContent(2) = ""
            Else
                RowContent(2) = EcotrophManager.BwdCalBiomass(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.BwdCalAccessBiomass(Row)) Then
                RowContent(3) = ""
            Else
                RowContent(3) = EcotrophManager.BwdCalAccessBiomass(Row).ToString("F4") ''''
            End If
            If Single.IsNaN(EcotrophManager.BwdCalFlow(Row)) Then
                RowContent(4) = ""
            Else
                RowContent(4) = EcotrophManager.BwdCalFlow(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.BwdCalKinetic(Row)) Then
                RowContent(5) = ""
            Else
                RowContent(5) = EcotrophManager.BwdCalKinetic(Row).ToString("F4")
            End If
            ...
            If Single.IsNaN(EcotrophManager.BwdCalFishLossRate(Row)) Then
                RowContent(7) = ""
            Else
                RowContent(7) = EcotrophManager.BwdCalFishLossRate(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.BwdCalAccessFishLossRate(Row)) Then
                RowContent(8) = ""
            Else
                RowContent(8) = EcotrophManager.BwdCalAccessFishLossRate(Row).ToString("F4") ''''
            End If
            ...
            End If
            If Single.IsNaN(EcotrophManager.BwdCalFishMortality(Row)) Then
                RowContent(10) = ""
            Else
                RowContent(10) = EcotrophManager.BwdCalFishMortality(Row).ToString("F4") ''''
            End If
            If Single.IsNaN(EcotrophManager.BwdCalAccessFishMortality(Row)) Then
                RowContent(11) = ""
            Else
                RowContent(11) = EcotrophManager.BwdCalAccessFishMortality(Row).ToString("F4")
            End If
            If Single.IsNaN(EcotrophManager.BwdCalSelectivity(Row)) Then
                RowContent(12) = ""
            Else
                RowContent(12) = EcotrophManager.BwdCalSelectivity(Row).ToString("F4") ''''
            End If
            If Row < EcotrophManager.BwdCalBiomass.GetUpperBound(0) Then
                If Single.IsNaN(EcotrophManager.BwdCalTime(Row)) Then
                    RowContent(13) = ""
                Else
                    RowContent(13) = EcotrophManager.BwdCalTime(Row).ToString("F4") ''''
                End If
            Else
                RowContent(13) = ""
            End If
